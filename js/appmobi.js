//keep reference to original document.dispatchEvent
var doc_dispatchEvent_orig = document.dispatchEvent;

//override document.dispatchEvent
document.dispatchEvent = function(e) {
	//first dispatch original event
	doc_dispatchEvent_orig.call(document, e);

	//if it is an appMobi.* event, make a intel.xdk.* event and dispatch
	if(e.type.indexOf('appMobi.') == 0) {
		var e2 = document.createEvent('Events');
		e2.initEvent(e.type.replace("appMobi.", "intel.xdk."), e.bubbles, e.cancelable);
		//shallow copy is good enough for all appMobi.* events
		for(var prop in e) {
			if(e.hasOwnProperty(prop)) {
				e2[prop] = e[prop];
			}
		}
		doc_dispatchEvent_orig.call(document, e2);
	}
}
...
AppMobi.app = 'i11.khetabeghadir';AppMobi.release = '4.0.0';AppMobi.webRoot = 'http://localhost:58888/i11.khetabeghadir/4.0.0/';AppMobi._hasAnalytics = true;AppMobi._hasCaching = true;AppMobi._hasStreaming = true;AppMobi._hasAdvertising = true;AppMobi._hasOAuth = false;AppMobi.oauthAvailable = false;
AppMobi.picturelist=["picture_000.png"];